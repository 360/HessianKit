...
-(BOOL)peek:(uint8_t*)buffer maxLength:(NSUInteger)length;
{
	uint8_t* streamBuffer = NULL;
  NSUInteger streamLength = 0;
  if ([self getBuffer:&streamBuffer length:&streamLength]) {
  	if (streamLength >= length) {
    	memcmp(buffer, streamBuffer, length);
      return YES;
    }
  }
  return NO;
...
{
	BOOL value;
  [self peek:(void*)&value maxLength:sizeof(value)];
  return value;
}
...
{
	unsigned char value;
  [self peek:&value maxLength:sizeof(value)];
  return value;
}
